...
export function SkinSettingsPanel({
  ...
  function addSkin() {
    setSaved(false);
    updateState((current) => {
      const id = `${makeSlugId("New Season Skin", "season_skin")}-${Date.now()}`;
      const nextSkin: JourneySkin = {
        id,
        name: "New Season Skin",
        status: "draft",
        description: "Reusable season visual package.",
        canDisable: true,
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-sm font-bold text-journey-steel">
            Skin Developer controls season visual packages. Turn skins off for
            the clean Cinema Standard experience, or draft future seasons like Dune 3.
          </p>
          {saved ? (
